...
class AsistenteReporteVentas(models.TransientModel):
    ...
    def print_report_excel(self):
        for w in self:
            ...
            hoja.write(y, 4, 'Cliente')
            hoja.write(y, 5, 'NIT')
            hoja.write(y, 6, 'Ventas')
            hoja.write(y, 7, 'Ventas exento')
            hoja.write(y, 8, 'Servicios')
            hoja.write(y, 9, 'Servicios exento')
            hoja.write(y, 10, 'Exportaciones')
            hoja.write(y, 11, 'IVA')
            hoja.write(y, 12, 'Total')

            for linea in lineas:
                y += 1
                hoja.write(y, 0, linea['tipo'])
                hoja.write(y, 1, linea['fecha'])
                hoja.write(y, 2, linea['serial'])
                hoja.write(y, 3, linea['numero'])
                hoja.write(y, 4, linea['cliente'])
                hoja.write(y, 5, linea['nit'])
                ...
                hoja.write(y, 10, linea['importacion']+linea['importacion_exento'])
                hoja.write(y, 11, linea['iva'])
            ...
            hoja.write(y, 10, totales['importacion']['neto']+totales['importacion']['exento'])
            hoja.write(y, 11, totales['compra']['iva'] + totales['servicio']['iva'] + totales['importacion']['iva'])
            hoja.write(y, 12, totales['compra']['total'] + totales['servicio']['total'] + totales['importacion']['total'])
